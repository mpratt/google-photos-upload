...
class OAuth:
    ...
    def authorize(self):
            ...
            if datetime.timestamp(now) <= expiration:
                self.session = OAuth2Session(
                    ...
                    auto_refresh_kwargs={
                        ...
                        "client_secret": self.credentials_data["installed"][
                            "client_secret"
                        ],
                    },
        ...
        flow.run_local_server(open_browser=False, bind_addr="0.0.0.0", port=8089)
    ...
    def make_upload(self, method, path, data, timeout):
        ...
        if timeout is None:
            ...
            return self.session.request(
                method, path, data=file.read(), headers=headers, timeout=timeout
            )
